...
class Repo:
    ...
    def get_deps(self, repo_vars: dict, path: str) -> None:
        print("evaluating " + json.dumps(self, default=vars, sort_keys=True), file=sys.stderr)
...
def update_all_source(commit: bool) -> None:
    ...
    repos = Parallel(n_jobs=2, require="sharedmem")(
        delayed(get_electron_info)(major_version)
        for major_version in supported_version_range()
    )
    new_info = {
        n[0]: n[1]
...
https://github.com/electron/electron/releases/tag/v{new_version}
"""
        subprocess.run(
            [
                "git",
...
@cli.command("eval", help="Print the necessary sources to fetch for a given major release")
